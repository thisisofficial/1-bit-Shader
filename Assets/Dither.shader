    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 edge (float2 uv, float2 delta){
                ...
                return float2(min(up.b, min(min(down.b, left.b), min(right.b, center.b))),
                max(max(distance(center.rg, up.rg), distance(center.rg, down.rg)),
                max(distance(center.rg, left.rg), distance(center.rg, right.rg))));
            }
